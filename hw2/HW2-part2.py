...
def main():
    ...
    """
    img = load_color_image("Nailpolish.jpeg")
    ...
    cv2.imwrite("Nailpolish_blurred.png", (blurred[:, :, ::-1] * 255).astype(np.uint8))
    cv2.imwrite("Nailpolish_restored.png", (restored[:, :, ::-1] * 255).astype(np.uint8))
    cv2.imwrite("Nailpolish_original.png", (img[:, :, ::-1] * 255).astype(np.uint8))
